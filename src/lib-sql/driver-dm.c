...
static const int dm_busy_timeout = 1000;

struct dm_db {
	struct sql_db api;

	pool_t pool;
    const char *user, *password,*dbname;
...
struct dm_transaction_context {
	struct sql_transaction_context ctx;
	bool failed:1;
};

extern const struct sql_db driver_dm_db;
extern const struct sql_result driver_dm_result;
extern const struct sql_result driver_dm_error_result;

static struct event_category event_category_dm= {
...
{
 	struct dm_db *db = (struct dm_db *)_db;

	if (db->connected)
		return 1;

    db->environment = Environment::creatEnvironment(Enviroment::DEFAULT);
    db->connection = db->environment->createConnection(db->user,db->password,db->dbname);

	if (db->environment != NULL && db->connection != NULL) {
		db->connected = TRUE;
		return 1;
	} else {
		e_error(_db->event, "open(%s) failed!", db->dbname);
        Environment::terminateEnvironment(db->environment);
		db->environment = NULL;
        db->connection = NULL;
		return -1;
	}
...
static void driver_dm_disconnect(struct sql_db *_db)
{
 	struct dm_db *db = (struct dm_db *)_db;
    db->environment->terminateConnection(db->connection);
    Environment::terminateEnvironment(db->environment);
    db->environment = NULL;
    db->connection = NULL;
}
				     ...
				     const char **error_r ATTR_UNUSED)
{
	...
	pool = pool_alloconly_create("dm driver", 512);
	db = p_new(pool, struct dm_db, 1);
	db->pool = pool;
	db->api = driver_dm_db;
	db->dbfile = p_strdup(db->pool, set->connect_string);
	db->connected = FALSE;
	db->api.event = event_create(set->event_parent);
	event_add_category(db->api.event, &event_category_dm);
	event_set_append_log_prefix(db->api.event, "dm: ");
...
{
	struct dm_db *db = (struct dm_db *)result->db;
	bool success = db->connected;
	int duration;
	const char *suffix = "";
...
static void driver_dm_exec(struct sql_db *_db, const char *query)
{
	struct dm_db *db = (struct sqlite_db *)_db;
